...
float euclidean_distance_2d(float x0, float x1, float y0, float y1, float z0, float z1)
{
...
byte run_knn(float pressure, float previous_pressure,  float delta)
{
  ...
  for (byte i = 0; i < K_NEIGHBORS; ++i) {
    neighbors_distance[i] = euclidean_distance_2d(pressure, dataset[i][0], previous_pressure, dataset[i][1], delta, dataset[i][2]);
  ...
  for (int i = K_NEIGHBORS; i < SIZE_DATASET; ++i) {
    float dist =  euclidean_distance_2d(pressure, dataset[i][0], previous_pressure, dataset[i][1], delta, dataset[i][2]);
...
void loop() {
  ...
  static float previous2_pressure = 0;
  ...
  if (pressure != 0) {
    ...
    previous2_pressure = previous_pressure;
    previous_pressure = pressure;
    byte rain = run_knn(pressure, previous2_pressure, delta);
    if(rain) {
      digitalWrite(LED_PIN_BLUE, LOW);
      analogWrite(LED_PIN_WHITE, 256);
    } else {
